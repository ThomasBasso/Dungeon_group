...
public class AdventureGame {
	...
	/**
	 * Permet de demander au joueur s'il souhaite continuer de jouer ou arrêter la partie
	 * @param nextLevel le prochain niveau du donjon
	 */
	protected void optionEndDungeon(Dungeon.TYPE nextLevel){
		System.out.println("Bravo vous venez de terminer le donjon " + this.dungeon.getActual_level()+ " " + new String(Character.toChars(0x1F3C6)));
		if (nextLevel != null) {
			System.out.println("Vous avez désormais le choix entre : \n 1 - Passer au donjon suivant \n 2 - Quitter la partie");
			Scanner scanner = new Scanner(System.in);
			...
			System.out.println(new String(Character.toChars(0x1F3C6)) + " Bravo vous avez terminé le jeu, vous êtes le meilleur " +
	...
	/**
	 * Permet de demander au joueur s'il souhaite enregistrer sa partie avant de partir s'il est encore en vie sinon le jeu se termine
	 */
	protected void optionEndGame() {
		if(this.player.getLifePoints() > 0 && this.dungeon.getNextLevel() != null) {
			System.out.println();
			System.out.println("Tu souhaites donc arrêter de jouer alors que tu viens de terminer le donjon, si tu le souhaites" +
